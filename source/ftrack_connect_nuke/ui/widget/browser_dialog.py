...
class BrowserDialog(BaseDialog):
    ...
    def __init__(self, task=None, parent=None):
        super(BrowserDialog, self).__init__(
            parent=parent, disable_tasks_list=True)
    ...
    def setupUI(self):
        ...
        self.main_container_layout.addWidget(location_frame)
    ...
    def call_project_browser(self, current_project):
        ...
        self._stackLayout.setCurrentWidget(
            self._projects_browsers[current_project.getId()])
...
class BrowserProject(QtGui.QScrollArea):
    ...
    def add_task(self, current_task, new_level=0):
        ...
        if len(children) > 0 or len(tasks) > 0:
            current_task_list = TaskList(
                self._scrollAreaWidgetContents, new_level)
            ...
            self._scrollAreaWidgetContents.layout().addWidget(
                current_task_list)
    ...
    def set_task(self, task, block_signal):
        ...
        for i in range(len(path_list)):
            ...
            item = browserlist.findItems(
                path_list[i], QtCore.Qt.MatchFixedString)[0]
