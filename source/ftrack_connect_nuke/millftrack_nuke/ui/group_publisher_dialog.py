...
class GroupPublisherDialog(BaseIODialog):
    ...
    def __init__(self, nodes, version_id):
        ...
        super(GroupPublisherDialog, self).__init__(
    ...
    """
        ...
        self._group_warning_box._warning_header.setTextFormat(
            QtCore.Qt.RichText)
    ...
    def set_group_error(self, error=None, button=None, choices=None):
        if error != None:
            self._group_error_box.set_error(
                error, button=button, choices=choices)
        else:
    ...
    def _validate_group(self):
        ...
        if len(self._nodes_graph.group_assets_versions()) == 1:
            ...
            error += "  - {0} [task: {1}]<br/>".format(
                group_asset.name, task_info)
