...
class SnapshotsWidget(QtGui.QWidget):
  ...
  def setupUI(self):
    ...
    self._no_snapshot.setStyleSheet("QFrame{background: #000; border:0px;}")
    layout_no_snapshot = QtGui.QHBoxLayout(self._no_snapshot)
    label_no_snapshot = QtGui.QLabel("No snapshot available...", self._no_snapshot)
    label_no_snapshot.setStyleSheet("color:#855")
    label_no_snapshot.setAlignment(QtCore.Qt.AlignCenter|QtCore.Qt.AlignVCenter)
    layout_no_snapshot.addWidget(label_no_snapshot)
    self._stackLayout.addWidget(self._no_snapshot)

    self._edit_buttons = SnapshotsEditButtons(False, False, self._snapshot_frame)
    ...
    self._edit_buttons.handscroll_mode_toggled.connect(self.set_handscroll_mode)
        ...
        if isinstance(widget, QtGui.QAction):
          ...
          for child in widget.children():
            if isinstance(child, QtOpenGL.QGLWidget) and child.isVisible():
              ...
              pixmaps[name] = QtGui.QPixmap.fromImage(qimage).copy(visible_rect)
              break
          continue
        else:
