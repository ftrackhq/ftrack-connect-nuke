...
class StatisticWidget(QtGui.QWidget):
  ...
  def _analyse_metas(self):
    ...
    for version in self._scene_version.getParent().getVersions():
      ...
      for elt, stat_tuple in meta_version.iteritems():
        ...
        # Legacy
        if elt == "All": elt = self._all_nodes_label
        ...
        if stat_tuple.__class__ not in [tuple, list]:
          self._version_metas[elt][version.getVersion()] = infos_version_dict
          continue
        ...
        self._version_metas[elt][version.getVersion()] = infos_version_dict.copy()
  ...
  def set_graph(self):
    ...
    if current_label not in self._graph_dict.keys():
      ...
      graph = GraphWidget(graph_node, version_nb, version_metas, is_in_tree, self)
...
class GraphWidget(QtGui.QWidget):
  def __init__(self, nodes_name, version_number, infos_dict=None, in_tree=True, parent=None):
    ...
    self._buttons = dict(total=None, enabled=None, in_tree=None, tracked=None)
    self._cursor = None
    self.raise_level = 7
  ...
  def paintEvent(self, event):
    ...
    rect = QtCore.QRect(self._pad_left, self._pad_top, width_graph, height_graph)
    ...
    unhighlight_graphs = [p for p in self._points.keys() if p != self._highlight_graph]
    ...
    width_interval = 1 if len(versions) == 1 else 1 / float(len(versions)-1)
    for i in versions:
    ...
    for type_graph in all_graphs_type:
      ...
      for i in sorted(pts_legend_X.keys(), reverse=True):
        ...
        try:
          ...
          ypos = padding - relative_position * graph_point.normalized_value
        ...
        # Draw vertical lines of the graph (and horizontal if necessary)
        color = self._color_dot_lines_hover if hover else self._color_dot_lines
        painter.setPen(QtGui.QPen(QtGui.QColor(color), 1, QtCore.Qt.DotLine))
        line_tuples = [ (pts_legend_X[i], self._pad_top),
                        (pts_legend_X[i], self.height() - self._pad_bottom) ]
        line_polys = QtGui.QPolygonF(map(lambda p: QtCore.QPointF(*p), line_tuples))
        painter.drawPolyline(line_polys)

        if hover:
          line_H = [ (self._pad_left, hover_point[1]),
                     (self._pad_left + width_graph, hover_point[1]) ]
          line_H_polys = QtGui.QPolygonF(map(lambda p: QtCore.QPointF(*p), line_H))
          painter.drawPolyline(line_H_polys)

      # Draw the lines of the graph
      width_line = 3 if highlighted else 2
      color = self._points_colors[type_graph] if highlighted else self._points_colors[type_graph].darker(110)
      painter.setPen(QtGui.QPen(QtGui.QColor(color), width_line))
      points_polys = QtGui.QPolygonF(map(lambda p: QtCore.QPointF(*p), points_tuples))
      painter.drawPolyline(points_polys)

      # Draw points
      for point_tuple in points_tuples:
        painter.drawEllipse(QtCore.QRectF(point_tuple[0]-2, point_tuple[1]-2, 4, 4))
    ...
    for version_nb in sorted(pts_legend_X.keys(), reverse=True):
      ...
      version_position_X = pts_legend_X[version_nb] - len(version_str) * 5
    ...
    for value_Y in sorted(pts_legend_Y.keys(), reverse=True):
      if buf_Y == 0 or pts_legend_Y[value_Y] - buf_Y > 15:
        ...
        painter.drawText(bounding_rect, QtCore.Qt.AlignRight, value_str)
    ...
    if hover_point != None:
      ...
      pos_topleft = QtCore.QPoint(self._cursor[0] + 5, self._cursor[1] + 5)
      if pos_topleft.x() + width_tooltip > self.width() - self._pad_right - 10:
        pos_topleft.setX(self.width() - self._pad_right - width_tooltip - 10)
      if pos_topleft.y() + height_tooltip > self.height() - self._pad_bottom - 10:
        pos_topleft.setY(self.height() - self._pad_bottom - height_tooltip - 10)
    ...
    for button_name in ["tracked", "in_tree", "enabled", "total"]:
      if self._buttons[button_name] != None:
        ...
        self._buttons[button_name].move(self.width()-width_btn-pad_left, pos_bottom)
        pad_left += width_btn + 5

        if self._highlight_graph == button_name:
          btn_css = "background:%s; color:#FFF; border:none; border-radius:3px;" % self._points_colors[button_name].name()
        else:
  ...
  def set_infos(self, infos_dict):
    ...
    for type_state in self._points.keys():
      ...
      for version, value in type_state_points:
        point = GraphPoint(self.nodes_name, type_state, version, value, self)
  ...
  def set_buttons(self):
    ...
    for name_btn, label_btn in btn_infos.iteritems():
      ...
      if self._points[name_btn] != None:
        ...
        self._buttons[name_btn].clicked.connect(self._toggle_highlight_graph)
...
class GraphPoint(object):
  ...
  def set_tooltip(self, value):
    if value == None:
      ...
      if value == 1:
        nb_nodes_str = "There is <strong>%d</strong> %s" % (value, self.name)
        nb_nodes_str = nb_nodes_str.replace("nodes", "node")
      elif value == 0:
        nb_nodes_str = "There are <strong>No</strong> %s" % (self.name)
      else:
        nb_nodes_str = "There are <strong>%d</strong> %s" % (value, self.name)
      version_str = "<strong>version %d</strong>" % self.version

      if self.type_state == "total":
        msg = "%s for the %s of this composition" % (nb_nodes_str, version_str)
        self.tooltip = "<span style=\"color:1e1e1e\">"+msg+"</span>"
      elif self.type_state == "enabled":
        msg = "%s enabled for the %s of this composition" % (nb_nodes_str, version_str)
        self.tooltip = "<span style=\"color:1e1e1e\">"+msg+"</span>"
      elif self.type_state == "in_tree":
        msg = "%s useful for the %s of this composition" % (nb_nodes_str, version_str)
        msg += "\n<i>(which belongs to a tree ending with an output)</i>"
        self.tooltip = "<span style=\"color:1e1e1e\">"+msg+"</span>"
      elif self.type_state == "tracked":
        msg = "%s tracked for the %s of this composition" % (nb_nodes_str, version_str)
        self.tooltip = "<span style=\"color:1e1e1e\">"+msg+"</span>"
