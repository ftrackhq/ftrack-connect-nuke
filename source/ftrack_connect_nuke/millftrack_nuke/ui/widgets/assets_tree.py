...
class TreeDelegateStyle(QtGui.QStyledItemDelegate):
  def __init__(self, parent, minimum_width=200, show_thumbnail=True):
    ...
    # Keep the width memory as option.rect is bullshitting in the sizeHint method...
    ...
    # Hover state
    self._hover_state = dict(no_hover= 0, btn_hover= 1, btn_pressed= 2, date_hover= 3)
  ...
  def paint(self, painter, option, index):
    ...
    if is_btn:
      ...
      self.draw_button(painter, text_btn, padding_left_btn, padding_top, index)
      ...
      if self._show_thumbnail:
        ...
        painter.setPen( QtGui.QPen(self._background.darker(190), 0.5, QtCore.Qt.SolidLine) )
        ...
        if self._is_top_asset_version(index):
          ...
          type_rect = QtCore.QRect( padding_left, padding_top_rect - 6,
          ...
          painter.setPen( QtGui.QPen(legend_text_color, 0, QtCore.Qt.SolidLine) )

          padding_top_legend = padding_top + thumbnail_rect.height() - 6
          painter.drawText( padding_left + 5, padding_top_legend, asset_type )

        padding_left += size_thumbnail.width() + 10

      # Draw locked symbol if necessary
      if locked:
        painter.drawPixmap(option.rect.right() - padding_right - 20, padding_top, self._icon_locked)
        padding_right += 30
      ...
      # If publisher is None, that means that the asset is being regenerated
      if publisher != None:

        # Draw publisher
        painter.setPen( QtGui.QPen(self._desc_color, 1, QtCore.Qt.SolidLine) )
        ...
        # Draw comment
        painter.setPen( QtGui.QPen(background_comment, 0, QtCore.Qt.SolidLine) )
        ...
        painter.setPen( QtGui.QPen(self._comment_color, 1, QtCore.Qt.SolidLine) )
      ...
      if self._need_button(index, version_nb):
        ...
        if publisher != None:
          padding_top += r_comment.height() + self._fm_desc.height() + self._space_before_btn
        else:
          padding_top += self._fm_desc.height() * 2 + self._space_before_btn


        padding_left_btn = option.rect.right() - self._fm_desc.width(text_btn)
        padding_left_btn -= (self._padding_item["right"] + self._padding_content["right"])

        self._button_rect = QtCore.QRect( padding_left_btn - 5,
                                          padding_top - self._fm_desc.height() - option.rect.top() - 5,
                                          self._fm_desc.width(text_btn) + 5,
  ...
  def sizeHint(self, option, index):
    ...
    height += self._padding_content["bottom"] + self._padding_item["bottom"]
  ...
  def draw_button(self, painter, text_btn, padding_left, padding_top, index):
    ...
    if btn_state == self._hover_state["btn_hover"]:
      painter.setPen( QtGui.QPen(self._btn_color_hover, 1, QtCore.Qt.SolidLine) )
    elif btn_state == self._hover_state["btn_pressed"]:
      painter.setPen( QtGui.QPen(self._btn_color_pressed, 1, QtCore.Qt.SolidLine) )
    else:
  ...
  def _bounding_boxes(self, comment, width, padding_left, padding_top):
    ...
    width_comment -= (self._padding_item["right"] + self._padding_content["right"])
    if width_comment < 0:
      width_comment = 0
    width_adjust = -self._padding_comment if width_comment >= self._padding_comment else 0
    ...
    bounding_text = self._fm_desc.boundingRect(r_comment_text, self._comment_flags, comment)
...
class AssetsTree(QtGui.QTreeView):
  ...
  def startDrag(self, supportedActions):
    ...
    if version is not None:
      ...
      mime_data.setData("text/plain", ":".join([id_ftrack, asset_name, version_id]))
  ...
  def import_assets(self, assets, asset_types=None, filter=None):
    ...
    self._controller.completed.connect(lambda: self.update_display(asset_types, filter))
    self._controller.start()
  ...
  def toggle_show_more_versions(self, btn_index):
    ...
    self._insert_versions(parent_index, last_asset_version, btn_index.row())
  ...
  def _insert_versions(self, parent_index, asset_version=None, start_number=0):
    ...
    while (i < start_number + self._add_versions_nb):
      ...
      version_item = AssetItem(self, asset_version, parent_item.asset_type())
      version_item.signal.asset_regenerated.connect(self._item_regenerated)
