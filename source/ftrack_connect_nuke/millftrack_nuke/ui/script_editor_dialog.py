...
class ScriptEditorDialog(BaseDialog):
  def __init__(self, scene_version):
    ...
    super(ScriptEditorDialog, self).__init__(QtGui.QApplication.activeWindow())
  ...
  def update_asset(self, scene_version):
    self._asset_name.setText(scene_version.name)
    self._asset_version.setText("%03d" % scene_version.version_number)
