...
class AssetsManager(object):
    ...
    def open_script(asset_version_id):
        ...
        if locker != None:
            ...
            AssetsManager.load_gizmos_from_task(
                scene_version.asset.task.ftrack_object)
    ...
    def publish_script_panel(self):
        ...
        if panel.result():
            ...
            asset = nuke_connector.create(
                panel.asset_name, task.ftrack_object, task.shot.ftrack_object)
    ...
    def load_gizmos_from_task(task):
        ...
        for gizmo in gizmos:
            for version in gizmo.versions:
                gizmo_version = N_AssetFactory.get_version(version, GizmoIO)
                gizmo_path = gizmo_version.path()
                if gizmo_path != None:
...
class RecentScenes(object):
    ...
    def add_scene(self, scene_version):
        ...
        name = scene_version.asset.task.parents + \
            " / " + scene_version.name + version_nb
